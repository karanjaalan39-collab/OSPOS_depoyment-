...
class Reports extends Secure_Controller
{
    ...
    // Skip permission check for best_sellers
    if ($method_name !== 'best_sellers') {
 ...
 * Best Sellers Report
 */
public function getBest_sellers(): void

{
    ...
    foreach ($results as &$row) {
        $row['pct_revenue'] = $grand_revenue > 0 ? round(($row['total_revenue'] / $grand_revenue) * 100, 1) : 0;
        $row['pct_qty']     = $grand_qty > 0 ? round(($row['total_qty'] / $grand_qty) * 100, 1) : 0;
        $row['margin']      = $row['total_revenue'] > 0 ? round(($row['total_profit'] / $row['total_revenue']) * 100, 1) : 0;
    }
    unset($row);

    $cat_rows   = $db->table('ospos_items')->select('category')->distinct()
        ->where('category !=', '')->where('deleted', 0)->orderBy('category', 'ASC')
        ->get()->getResultArray();
    ...
    $data = [
        ...
        'chart_labels'  => json_encode(array_map(fn($r) => $r['item_name'], array_slice($results, 0, 10))),
        'chart_qty'     => json_encode(array_map(fn($r) => (float)$r['total_qty'], array_slice($results, 0, 10))),
        'chart_revenue' => json_encode(array_map(fn($r) => (float)$r['total_revenue'], array_slice($results, 0, 10))),
    ];
