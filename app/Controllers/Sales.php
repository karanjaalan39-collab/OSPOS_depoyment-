...
class Sales extends Secure_Controller
{
    ...
    public function postAddPayment(): void
    {
        ...
        if (!$this->validate($rules, $messages)) {
            ...
            if ($payment_type === lang('Sales.giftcard')) {
                ...
                $amount_tendered = parse_decimals($this->request->getPost('amount_tendered'));
                $mpesa_phone     = $this->request->getPost('mpesa_phone', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
                $mpesa_receipt   = $this->request->getPost('mpesa_receipt', FILTER_SANITIZE_FULL_SPECIAL_CHARS);

                if (!empty($mpesa_receipt)) {
                    $current_comment = $this->sale_lib->get_comment();
                    $this->sale_lib->set_comment($current_comment . ' | MPesa Ref: ' . $mpesa_receipt . ' | Phone: ' . $mpesa_phone);
                }
        ...
        if ($mode == 'return' && $this->sale->is_valid_receipt($item_id_or_number_or_item_kit_or_receipt)) {
            ...
            if (!empty($kit_item_id)) {
                if (!$this->sale_lib->add_item($kit_item_id, $item_location, $quantity, $discount, $discount_type, PRICE_MODE_KIT, $kit_price_option, $kit_print_option, $price)) {
                    ...
                    $data['warning'] = $this->sale_lib->out_of_stock($kit_item_id, $item_location);
                }
            }
            ...
            if ($item_id_or_number_or_item_kit_or_receipt == '') {
                ...
                $item_info = $this->item->get_info_by_id_or_number($item_id_or_number_or_item_kit_or_receipt);

                if (!empty($item_info) && (int)$item_info->stock_type === HAS_STOCK) {
                    $stock_qty       = (float)$this->item_quantity->get_item_quantity((int)$item_info->item_id, $item_location)->quantity;
                    $already_in_cart = (float)$this->sale_lib->get_quantity_already_added((int)$item_info->item_id, $item_location);
                    $available       = $stock_qty - $already_in_cart;
                    $qty_requested   = (float)$quantity;

                    if ($qty_requested > $available) {
                        $data['error'] = '❌ Cannot add ' . (int)$qty_requested . ' unit(s) of "' . $item_info->name . '". Only ' . max(0, (int)$available) . ' unit(s) in stock.';
                        $this->_reload($data);
                        return;
                    }
                }
                ...
                if (!$this->sale_lib->add_item($item_id_or_number_or_item_kit_or_receipt, $item_location, $quantity, $discount, $discount_type, PRICE_MODE_STANDARD, null, null, $price)) {
                    ...
                    $stock_message = $this->sale_lib->out_of_stock($item_id_or_number_or_item_kit_or_receipt, $item_location);
                    if (!empty($stock_message)) {
                        $data['warning'] = $stock_message;
                    }
                }
            }
        }
     ...
     */
    /**
 * Complete and finalize a sale. CLEANED & FIXED version.
 *
 * @return void
 * @throws ReflectionException
 */
public function postComplete(): void
{
    ...
    // Common data for ALL receipt types
    ...
    $data['employee']              = $this->employee->get_logged_in_employee_info()->first_name . ' ' . mb_substr($this->employee->get_logged_in_employee_info()->last_name, 0, 1);
    $data['company_info']          = implode("\n", [$this->config['address'], $this->config['phone']]);
    if ($this->config['account_number']) $data['company_info'] .= "\n" . lang('Sales.account_number') . ": " . $this->config['account_number'];
    if ($this->config['tax_id'])         $data['company_info'] .= "\n" . lang('Sales.tax_id') . ": " . $this->config['tax_id'];
    ...
    $this->_load_customer_data($customer_id, $data);
    ...
    $data['amount_due']          = $data['cash_mode'] ? $totals['cash_amount_due'] : $totals['amount_due'];
    $data['amount_change']       = $data['amount_due'] * -1;

    // === FORCE NO PRINT if button was used ===
    $no_print = $this->request->getPost('no_print') == '1';
    if ($no_print) {
    ...
    // ====================== MODE-SPECIFIC LOGIC (CLEAN) ======================
    if ($this->sale_lib->is_invoice_mode()) {
        $data['mode_label'] = lang('Sales.invoice');
        if ($invoice_number === null) {
            $invoice_number = $this->token_lib->render($this->config['sales_invoice_format']);
        }
    ...
    elseif ($this->sale_lib->is_work_order_mode()) {
        ...
        $sale_status = $data['price_work_orders'] ? COMPLETED : SUSPENDED;
    ...
    if ($no_print) {
        ...
        header('Location: ' . site_url('sales'));
        exit;
    }

    // === Different views per mode ===
    if ($this->sale_lib->is_work_order_mode()) {
        echo view('sales/work_order', $data);
    } elseif ($this->sale_lib->is_quote_mode()) {
        echo view('sales/quote', $data);
    } else {
        // NORMAL RECEIPT (this is where your supermarket receipt goes)
        echo view('sales/receipt', $data);
    }

    $this->sale_lib->clear_all();   // ← MUST be AFTER echo
}
    ...
    private function _reload(array $data = []): void    // TODO: Hungarian notation
    {
        ...
        $data['payment_options']['mpesa'] = 'M-pesa';
