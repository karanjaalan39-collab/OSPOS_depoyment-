  ...
  public function out_of_stock(int $item_id, int $item_location): string
{
    ...
    if ($item_id != -1) {
        ...
        if ($item_info->stock_type == HAS_STOCK) {
            ...
            $available = $item_quantity - $quantity_added;

            if ($available < 0) {
                return '❌ Cannot add item "' . $item_info->name . '". Only ' . max(0, (int)$item_quantity) . ' unit(s) available in stock.';
            } elseif ($available < $item_info->reorder_level) {
                return '⚠️ Low stock on "' . $item_info->name . '". Only ' . (int)$available . ' unit(s) remaining.';
            }
        }
    }
