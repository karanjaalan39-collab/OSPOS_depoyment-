            ...
            <div id="payment_details">
                <?php if ($payments_cover_total) { // Show Complete sale button instead of Add Payment if there is no amount due left ?>
                    ...
                    <div class="btn btn-sm btn-success pull-right" id="finish_sale_button" tabindex="<?= ++$tabindex ?>">
                     ...
                     <div class="btn btn-sm btn-warning pull-right" id="finish_sale_no_print_button" tabindex="<?= ++$tabindex ?>" style="margin-right:5px;">
                        <span class="glyphicon glyphicon-save">&nbsp;</span>Save (No Print)
                    </div>     
                    <?php
                        }
                    }
                ...
                <?php } else { ?>
                    <?= form_open("$controller_name/addPayment", ['id' => 'add_payment_form', 'class' => 'form-horizontal']) ?>
                        ...
                        <!-- M-PESA PHONE INPUT - shows only when M-Pesa is selected -->
                        <div id="mpesa_phone_section" style="display:none; margin-top:5px;">
                            <div class="input-group input-group-sm">
                                <span class="input-group-addon">📱</span>
                                <input type="text"
                                       id="mpesa_phone_input"
                                       name="mpesa_phone"
                            ...
                            <div id="mpesa_push_status" style="margin-top:5px; font-size:12px;"></div>
                            <input type="hidden" name="mpesa_receipt" id="mpesa_receipt_hidden" value=""/>
                        </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#finish_sale_no_print_button').click(function() {
             if ($('#no_print_input').length === 0) {
                 $('#buttons_form').append('<input type="hidden" id="no_print_input" name="no_print" value="1">');
             }
             $('#buttons_form').attr('action', "<?= site_url("$controller_name/complete") ?>");
             $('#buttons_form').submit();
             });
...
// Show phone input when M-Pesa is selected
$('#payment_types').on('change', function() {
    var selectedPayment = $(this).val();
    var hasCustomer     = $('#customer').val() !== '<?= lang(ucfirst($controller_name) . '.start_typing_customer_name') ?>';

    // ── WARN IF DUE SELECTED WITHOUT CUSTOMER ──
    if (selectedPayment === '<?= lang('Sales.due') ?>' && !hasCustomer) {
        alert('⚠️ Please select a customer before using Due payment!');
        $(this).val('<?= lang('Sales.cash') ?>');
        selectedPayment = '<?= lang('Sales.cash') ?>';
    }

    // ── SHOW/HIDE MPESA PHONE INPUT ──
    if (selectedPayment === 'mpesa') {
        $('#mpesa_phone_section').show();
        $('#amount_tendered').val('<?= to_currency_no_money($amount_due) ?>');
    } else {
        $('#mpesa_phone_section').hide();
        $('#mpesa_push_status').html('');
    }
});

var mpesaPollTimer = null;

// Send STK Push when button clicked
$('#btn_send_mpesa').on('click', function() {
    var phone  = $('#mpesa_phone_input').val().trim();
    var amount = $('#amount_tendered').val().replace(/[^0-9.]/g, '');

    if (!phone || phone.length < 9) {
        $('#mpesa_push_status').html('<span style="color:red;">⚠️ Enter a valid phone number</span>');
        return;
    }

    $('#mpesa_push_status').html('<span style="color:orange;">⏳ Sending push to ' + phone + '...</span>');
    $('#btn_send_mpesa').prop('disabled', true);

    $.ajax({
        url: '<?= site_url("mpesa/pay") ?>',
        method: 'POST',
        data: {
            phone: phone,
            amount: Math.ceil(parseFloat(amount)),
            reference: 'OSPOS-<?= $sale_id ?? time() ?>'
        },
        dataType: 'json',
        success: function(res) {
            if (res.success) {
                $('#mpesa_push_status').html(
                    '<span style="color:green;">✅ Prompt sent! Waiting for customer to pay...</span>'
                );
                startMpesaPolling(phone);
            } else {
                $('#mpesa_push_status').html(
                    '<span style="color:red;">❌ ' + res.message + '</span>'
                );
                $('#btn_send_mpesa').prop('disabled', false);
            }
        },
        error: function() {
            $('#mpesa_push_status').html('<span style="color:red;">❌ Request failed. Try again.</span>');
            $('#btn_send_mpesa').prop('disabled', false);
        }
    });
});

// Poll every 5 seconds to check if customer paid
function startMpesaPolling(phone) {
    var count = 0;
    mpesaPollTimer = setInterval(function() {
        ...
        if (count > 24) {
            ...
            $('#mpesa_push_status').html('<span style="color:red;">⏱️ Timed out. Ask customer to check phone.</span>');
            $('#btn_send_mpesa').prop('disabled', false);
            return;
        }

        $.get('<?= site_url("mpesa/checkPayment") ?>', { phone: phone }, function(res) {
            if (res.paid) {
                clearInterval(mpesaPollTimer);

                // Store receipt
                $('#mpesa_receipt_hidden').val(res.receipt);

                // Fill in the amount
                $('#amount_tendered').val(res.amount);

                $('#mpesa_push_status').html(
                    '<span style="color:green; font-weight:bold;">' +
                    '✅ PAID! Receipt: ' + res.receipt + ' | KES ' + res.amount +
                    '</span>'
                );

                // Auto click Add Payment after 1.5 seconds
                setTimeout(function() {
                    $('#add_payment_button').click();
                }, 1500);
            }
